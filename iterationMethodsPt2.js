...
******************************************************************/
let tempGraterThan24 = temperatures.filter((temp) => temp > 24);

console.log(tempGraterThan24);

let tempLessThan20 = temperatures.filter((temp) => temp < 20);
...
******************************************************************/
let convertToFahrenhite = temperatures.map((C) => (C * 9) / 5 + 32);
console.log(convertToFahrenhite);

let labelOfTemp = temperatures.map((temp) => {
  if (temp >= 25) {
    return "Warm";
...
// Note: Use appropriate JavaScript array iteration methods such as filter,
//map, reduce, etc., to achieve the desired output for each task.
let maxTemp = temperatures[0];
let highestTempOfMOnth = temperatures.forEach((temp) => {
  if (temp > maxTemp) {
    maxTemp = temp;
  }
});
...
let lowestTempOfMOnth = temperatures.forEach((temp) => {
  if (temp < minTemp) {
    minTemp = temp;
  }
});
...
// let highestTempOfMOnth = temperatures.reduce((prevtemp, newtemp) => {
//   if (newtemp > prevtemp) {
//     return newtemp;
//   } else {
//     return prevtemp;
...
// let lowestTempOfMOnth = temperatures.reduce((prevtemp, newtemp) => {
//   if (newtemp < prevtemp) {
//     return newtemp;
//   } else {
//     return prevtemp;
